...
(function (gameLogic) {
    ...
    */
    function createMove(board, deltaFrom, deltaTo, turnIndexBeforeMove, isUnderCheck, canCastleKing, canCastleQueen, enpassantPosition, promoteTo) {
        ...
        if (getWinner(board, turnIndexBeforeMove, isUnderCheck, canCastleKing, canCastleQueen, enpassantPosition) !== '' ||
        ...
        var firstOperation;
        if (winner !== '' || isTie(boardAfterMove, turnIndexAfterMove, isUnderCheckAfterMove, canCastleKingAfterMove, canCastleQueenAfterMove, enpassantPositionAfterMove)) {
            // Game over.
            firstOperation = { endMatch: { endMatchScores: (winner === 'W' ? [1, 0] : (winner === 'B' ? [0, 1] : [0, 0])) } };
        }
        else {
    ...
    */
    function isCastlingKing(board, deltaFrom, deltaTo, turnIndex, canCastleKing) {
        var fromRow = deltaFrom.row, fromCol = deltaFrom.col, toCol = deltaTo.col, caslingRow = (turnIndex === 0 ? 7 : 0), turn = getTurn(turnIndex);
        ...
        if (canCastleKing[turnIndex] && fromRow === caslingRow &&
            ...
            for (var j = 5; j <= 6; j++) {
                ...
                if (isPositionUnderAttack(board, turnIndex, { row: fromRow, col: j })) {
                    return false;
                }
            }
    ...
    */
    function canKingMoveAnywhere(board, turnIndex, startPos, isUnderCheck, canCastleKing, canCastleQueen) {
        return getKingPossibleMoves(board, turnIndex, startPos, isUnderCheck, canCastleKing, canCastleQueen).length !== 0;
    }
    /**
    ...
    */
    function getKingPossibleMoves(board, turnIndex, startPos, isUnderCheck, canCastleKing, canCastleQueen) {
        var fromRow = startPos.row, fromCol = startPos.col, destinations = [], opponent = getOpponent(turnIndex);
        // standard moves
        for (var i = fromRow - 1; i <= fromRow + 1; i++) {
            for (var j = fromCol - 1; j <= fromCol + 1; j++) {
                if (i >= 0 && i <= 7 && j >= 0 && j <= 7) {
    ...
    */
    function getRookPossibleMoves(board, turnIndex, startPos) {
        var fromRow = startPos.row, fromCol = startPos.col, toPos = [];
        for (var i = 1; i <= 7; i++) {
            var endPos1 = { row: fromRow + i, col: fromCol }, endPos2 = { row: fromRow - i, col: fromCol }, endPos3 = { row: fromRow, col: fromCol + i }, endPos4 = { row: fromRow, col: fromCol - i };
    ...
    */
    function getBishopPossibleMoves(board, turnIndex, startPos) {
        var fromRow = startPos.row, fromCol = startPos.col, toPos = [];
        for (var i = 1; i <= 7; i++) {
            var endPos1 = { row: fromRow - i, col: fromCol - i }, endPos2 = { row: fromRow - i, col: fromCol + i }, endPos3 = { row: fromRow + i, col: fromCol - i }, endPos4 = { row: fromRow + i, col: fromCol + i };
    ...
    function getKnightPossibleMoves(board, turnIndex, startPos) {
        ...
        for (var i = fromRow - 2; i <= fromRow + 2; i++) {
            if (i !== fromRow) {
                ...
                if (canKnightMove(board, startPos, endPos1, turnIndex)) {
                    toPos.push(endPos1);
                }
                if (canKnightMove(board, startPos, endPos2, turnIndex)) {
                    toPos.push(endPos2);
                }
                if (canKnightMove(board, startPos, endPos3, turnIndex)) {
                    toPos.push(endPos3);
                }
                if (canKnightMove(board, startPos, endPos4, turnIndex)) {
                    toPos.push(endPos4);
                }
            }
        }
    ...
    */
    function canPawnMove(board, deltaFrom, deltaTo, turnIndex, enpassantPosition) {
        var fromRow = deltaFrom.row, fromCol = deltaFrom.col, toRow = deltaTo.row, toCol = deltaTo.col, turn = getTurn(turnIndex);
        if (toRow < 0 || toCol < 0 || toRow > 7 || toCol > 7) {
            return false;
        }
        var endPiece = board[toRow][toCol];
    ...
    */
    function getPawnPossibleMoves(board, turnIndex, startPos, enpassantPosition) {
        var fromRow = startPos.row, fromCol = startPos.col, toPos = [], turn = getTurn(turnIndex), endRow;
        for (var j = fromCol - 1; j <= fromCol + 1; j++) {
    ...
    */
    function moveAndCheck(board, turnIndex, startPos, endPos) {
        var fromRow = startPos.row, fromCol = startPos.col, toRow = endPos.row, toCol = endPos.col, opponent = getOpponent(turnIndex);
        if (board[toRow][toCol] === opponent + 'K') {
            return true;
        }
    ...
    */
    ...
    function isMoveOk(params) {
        ...
        try {
            ...
            var expectedMove = createMove(board, deltaFrom, deltaTo, turnIndexBeforeMove, isUnderCheck, canCastleKing, canCastleQueen, enpassantPosition, promoteTo);
            // console.log("jane!!!!");
    ...
    function getPossibleMoves(board, turnIndex, isUnderCheck, canCastleKing, canCastleQueen, enpassantPosition) {
        ...
        var possibleMoves = [], turn = (turnIndex === 0 ? 'W' : 'B');
        for (var i = 0; i <= 7; i++) {
            for (var j = 0; j <= 7; j++) {
                var piece = board[i][j];
